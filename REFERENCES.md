...
`data/ccps/*.json`, `data/heat-sources/*.json`,
`data/meal-pattern-contributions/*.json`, `data/foodon-crosswalk/*.json`,
and `src/*.ts` (not retyped from memory — see the extraction command in
this file's own git history if you want to re-run the check), plus
everything researched live via `WebSearch`/`WebFetch` this session and
written into a real file. Each entry states **where** it's used and **how
confident** this repo is, matching `CitationSchema`'s own `confidence`
vocabulary (`standard_reference` / `commonly_cited_unverified`) rather
than presenting everything as equally certain.

**Section index** (added 2026-08-19 as this file crossed ~1,200 lines —
same "a long flat file needs real navigation" standard `LEARNINGS.md` was
already held to in August; unlike that file, this one's actual CONTENT
didn't need restructuring — each entry here documents one distinct,
still-accurate citation tied to a specific field, not repeated narrative,
so the fix is navigability, not deduplication):

1. [Food-safety / regulatory standards](#food-safety--regulatory-standards) — every HACCP/CCP threshold's legal basis
2. [Physical constants & composition data](#physical-constants--composition-data) — thermophysical, compositional, and physics citations
3. [Culinary technique & food science literature](#culinary-technique--food-science-literature) — technique, timing, and food-science-mechanism citations
4. [Discussed, informs design reasoning, not yet embedded in any data file](#discussed-informs-design-reasoning-not-yet-embedded-in-any-data-file)
5. [Robotic / automated cooking systems](#robotic--automated-cooking-systems) — kept separate from safety/physics on purpose, see its own note
6. [Interoperability formats](#interoperability-formats) — Cooklang, Schema.org
7. [USDA meal-pattern crediting](#usda-meal-pattern-crediting-srcnutrition-extensionts) — `src/nutrition-extension.ts`
8. [FoodOn ontology crosswalk](#foodon-ontology-crosswalk-srcfoodon-crosswalkts) — `src/foodon-crosswalk.ts`
9. [Simulation / robot-execution research](#simulation--robot-execution-research) — points at `SIMULATION_TARGETS.md`, not duplicated here
10. [What's original vs. what's cited](#whats-original-vs-whats-cited)
...
## FoodOn ontology crosswalk (`src/foodon-crosswalk.ts`)

- **FoodOn** (foodon.org) — https://foodon.org/, an [OBO Foundry](https://obofoundry.org/ontology/foodon)
  ontology (CC BY 4.0) for standardized food classification, "farm to
  fork." `data/foodon-crosswalk/*.json` maps 22 of this repo's 42
  entities to their matching FoodOn class, matched via direct, live
  queries against FoodOn's own [EBI Ontology Lookup Service (OLS4)](https://www.ebi.ac.uk/ols4/ontologies/foodon)
  search API, 2026-08-19 — FoodOn's own recommended browsing/query tool,
  not a secondary aggregator. `confidence: "standard_reference"` for 17
  clean, unambiguous matches (potato, egg, egg_yolk, egg_white, egg_shell,
  garlic, onion, salt, oil, sunflower_oil, milk, butter, flour,
  baking_soda, potato_peel, dough, vinegar); `confidence:
  "commonly_cited_unverified"` for 5 best-available-but-imperfect matches
  (black_pepper, chili_flakes, flaky_salt, yeast, water — each with an
  explicit per-entity `note` on the specific caveat, e.g. FoodOn having no
  flake-specific salt class, or two near-duplicate "bakers yeast" classes
  with no disambiguating description found this session). See
  `reference/foodon-crosswalk.md` for the full reasoning, including which
  20 entities have NO crosswalk entry and why (13 tool entities,
  categorically out of scope since FoodOn doesn't model cookware; 4
  ingredients — `kosher_salt`/`onion_peel`/`garlic_peel`/`egg_cracked` —
  with no FoodOn class found that cleanly matches; 3 dish-specific
  composite mixtures left unmatched for the same reason `nutrition-
  extension.ts` leaves composites uncredited).
