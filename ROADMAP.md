...
| **Comments → code: an expansive sweep of all 31 `src/*.ts` files** promoted real, checkable facts that were sitting only in doc-comment prose into typed/cited data — `ROOM_TEMP_C` (one constant replacing 4 independent magic-number literals), `altitude.ts`'s barometric/Antoine validity bounds (now checkable, matching `thermal.ts`'s own `validityCondition` precedent), the Maillard reaction's full temperature curve + starch gelatinization onset (`heat-penetration.ts`), 3 new queryable `domainFacts` (`potato.starchToSugarThresholdC`, `egg.euGradeLRangeGrams`/`usGradeLargeMinGrams`), real `roomTempHours` (USDA Danger Zone) populated on 4 genuinely perishable entities, and a formal `ALLERGEN_REGULATION_CITATION` constant | ✅ Makeable, closed 2026-08-18 | `npm run capability-test:comments-to-code` |
