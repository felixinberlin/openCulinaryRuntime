...
console.log("=== 3. A raw (unwashed) potato still can't be cut — the prerequisite still means something ===");
try {
  applyAction({ entityId: "potato", state: "raw", tags: [] }, cut, entities, tools, { shape: "diced" });
...
const mashedFromBoiled = applyAction(boiledPotato, mash, entities, tools, { consistency: "smooth" }).instance;
console.log(`  BOIL then MASH: "peeled" -> "${boiledPotato.state}" -> "${mashedFromBoiled.state}"`);

const bakedPotato = applyAction(peeled, bake, entities, tools, { durationSeconds: "3600" }).instance;
const mashedFromBaked = applyAction(bakedPotato, mash, entities, tools, { consistency: "chunky" }).instance;
console.log(`  BAKE then MASH: "peeled" -> "${bakedPotato.state}" -> "${mashedFromBaked.state}"\n`);

console.log("=== 6. A raw potato still can't be mashed — MASH's prerequisite is real, not decorative ===");
try {
  applyAction({ entityId: "potato", state: "raw", tags: [] }, mash, entities, tools, { consistency: "smooth" });
  console.log("  UNEXPECTED: raw potato was mashable");
} catch (err) {
  console.log(`  REJECTED as expected:\n    ${(err as Error).message}\n`);
}

console.log("=== 7. Mashed potato can still be FRYed — zero further engine changes needed (FRY has no prerequisite) ===");
const friedMash = applyAction(mashedFromBoiled, fry, entities, tools, { oilTempC: "180", durationSeconds: "300" }, ingredients).instance;
console.log(`  FRY: "${mashedFromBoiled.state}" -> "${friedMash.state}" — a simple potato-cake/leftover-mash technique, real and representable\n`);
