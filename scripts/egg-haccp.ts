...
console.log(
  `Completed: state "${humanFlash.instance.state}". The warning is informational — a human reads it and judges.`
);
...
try {
  ...
  console.log(
    `REJECTED as expected — no human present to accept this risk:\n  ${(err as Error).message}`
  );
}
...
fry({ entityId: "egg", state: "raw", tags: [] }, 10, {
  mode: "autonomous",
  humanOverrides: new Set(["egg_cooking"]),
});
