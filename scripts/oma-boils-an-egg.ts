...
console.log("(That's the entire request — no temperature, no timing, no heat source, no safety detail.)\n");
...
console.log(`   isBoilable: ${egg.capabilities.isBoilable}, isSimmerable: ${egg.capabilities.isSimmerable}`);
console.log(`   Safety threshold wired to BOIL: "${egg.criticalControlPointsByAction.boil}" (data/ccps/egg_cooking.json)\n`);
...
console.log(`2. "medium" resolved via EGG_BOIL_DONENESS: ${doneness.durationSecondsRange.min}-${doneness.durationSecondsRange.max}s`);
...
const preheatSeconds = estimatedPreheatSeconds(potWaterMassKg, startTempC, water.thermophysical!.boilingPointC!, heatSource);
console.log(`3. Kitchen has: ${heatSource.names.en}. Water reaches boiling in ~${preheatSeconds.toFixed(0)}s from ${startTempC}°C —`);
console.log(`   a real, computed number (heat-source.ts), not asked of Oma or guessed by the robot.\n`);
...
console.log(`4. Execution mode: "${policy.mode}" — no human present to judge a shortfall, so the safe default applies automatically.\n`);
...
const boiled = applyAction(rawEgg, actions.get("boil")!, entities, tools, { durationSeconds: String(durationSeconds) }, ingredients, ccps, policy);
console.log(`5. BOIL executed: "${rawEgg.state}" -> "${boiled.instance.state}"`);
console.log(`   Warnings: ${boiled.warnings.length === 0 ? "none — comfortably clears egg_cooking's threshold" : boiled.warnings.join("; ")}\n`);
...
const shocked = applyAction(boiled.instance, actions.get("shock")!, entities, tools, {}, ingredients).instance;
const peeled = applyAction(shocked, actions.get("peel")!, entities, new Set(["knife"]), {}, ingredients).instance;
console.log(`6. SHOCK then PEEL: "${boiled.instance.state}" -> "${shocked.state}" -> "${peeled.state}"\n`);
