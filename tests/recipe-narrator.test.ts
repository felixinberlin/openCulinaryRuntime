...
describe("narrateRecipe", () => {
  ...
  test("createdElements lists only spawned instances, with inherited tags surfaced", () => {
    ...
    assert.ok(
      !n.createdElements.some((e) => e.instanceId === "potato-1" || e.instanceId === "oil-1")
    );
  });
  ...
  test("a created element that's re-targeted AFTER spawning does NOT claim its later tags were inherited (regression: tortilla_mixture/FLIP case)", () => {
    ...
    const mixture = makeEntity({
      ...
      possibleTags: ["flipped"],
      capabilities: { isFryable: true },
    });
    const localEntities = new Map([...entities, ["mixture", mixture]] as [
      string,
      ReturnType<typeof makeEntity>,
    ][]);
    const combine = makeAction({
      id: "combine",
      requiredSecondaryCapability: "isFryingMedium",
      ...
      sequence: [
        {
          actionId: "combine",
          targetInstanceId: "potato-1",
          secondaryInstanceId: "oil-1",
          params: {},
          availableIngredientInstanceIds: [],
        },
        {
          actionId: "flip",
          targetInstanceId: "mixture-1",
          params: {},
          availableIngredientInstanceIds: [],
        },
      ],
    ...
    assert.deepEqual(
      spawned.confidentlyInheritedTags,
      [],
      "flipped was NOT present at spawn time, must not be claimed as inherited"
    );
  });
