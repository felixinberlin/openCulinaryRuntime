...
describe("buildRecipeScaffold", () => {
  const potato = makeEntity({ id: "potato", possibleStates: ["raw", "peeled"], capabilities: { isFryable: true } });
  const oil = makeEntity({ id: "oil", possibleStates: ["cold", "hot"], capabilities: { isFryingMedium: true } });
  ...
  test("derives id and names.en from the slug, same convention every data/recipes/*.json file uses", () => {
    const scaffold = buildRecipeScaffold({ slug: "quick-fried-potatoes", entityIds: ["potato"] }, entities);
  ...
  test("numbers instance ids PER ENTITY TYPE, not with one shared global counter (regression)", () => {
    ...
    const scaffold = buildRecipeScaffold({ slug: "test", entityIds: ["potato", "oil", "potato"] }, entities);
    assert.deepEqual(
  ...
  test("throws on an unknown entity id rather than producing a silently-broken scaffold", () => {
    assert.throws(() => buildRecipeScaffold({ slug: "test", entityIds: ["potatoo"] }, entities), /unknown entity id "potatoo"/);
  });

  test("throws when given zero entity ids (RecipeScriptSchema requires initialInventory.min(1))", () => {
    assert.throws(() => buildRecipeScaffold({ slug: "test", entityIds: [] }, entities), /at least one entity id is required/);
  });
});
